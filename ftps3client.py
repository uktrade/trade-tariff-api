...
def s3_to_ftps(folder):
    logger.info("s3_to_ftps: %s", folder)
    try:
        ...
        for file in s3files:
            ...
            if fn != "" and "/success/" not in f:
                # ignore object that is just the folder name and already processed files
                logger.info("Uploading file %s", f)

                ftp.storbinary(
                    "STOR " + fn, get_file(f)
                )  # get_file gives us the readable stream
...
def ftps_to_s3(folder):
    logger.info("ftps_to_s3: %s", folder)

    try:
        ftp.cwd(FTPUSERROOT + folder)
        logger.info("Changed directory to %s", ftp.pwd())
        ...
        for d in dirlines:
            if d[0] != "d":  # d indicates directory
                ...
                # Move retrieved file
                ftp.rename(
                    FTPUSERROOT + folder + "/" + filename,
                    FTPUSERROOT + folder + "/success/" + filename,
                )
    except all_errors as e:
        logger.exception("%s", e)
